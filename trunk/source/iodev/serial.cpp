    ...
    if (SIM->get_param_bool("enabled", base)->get()) {
      ...
      if (!strcmp(mode, "file")) {
       ...
       sin.sin_len = sizeof sin;
        ...
        // server mode
        if (server) {
          if (net_bind(socket, (sockaddr *) &sin, sizeof (sin)) < 0 ||
              net_listen(socket, 0) < 0) { //SOMAXCONN --> I don't know what here must stand
            closesocket(socket);
            socket = -1;
            ...
            int client;
            if ((client = net_accept(socket, NULL, 0)) < 0)
        ...
        // client mode
        else if (net_connect(socket, (sockaddr *) &sin, sizeof (sin)) < 0) {
          closesocket(socket);
          socket = -1;
